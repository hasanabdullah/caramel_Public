                        ...
                        if (event.allDay) {
                            ...
                            if (eventEndTime <= startTime || eventStartTime >= endTime) {
                                ...
                                do {
                                    ...
                                    if (displayEvent.event.create == true) {
                                        if (displayEvent.event.title.substr(0, 2) == "**") {
                                            displayEvent.event.title = displayEvent.event.title;
                                        } else {
                                            displayEvent.event.title = "**" + displayEvent.event.title;
                                        }
                                        /*scope.repairorder={
...
angular.module("template/rcalendar/week.html", []).run(["$templateCache", function ($templateCache) {
    $templateCache.put("template/rcalendar/week.html",
        ...
        "                        <div ng-repeat=\"displayEvent in tm.events\" class=\"calendar-event\"\n" +
        "                             ng-click=\"eventSelected({event:displayEvent.event})\"\n" +
        "                             ng-style=\"{top: (37*displayEvent.startOffset/hourParts)+'px',left: 100/displayEvent.overlapNumber*displayEvent.position+'%', width: 100/displayEvent.overlapNumber+'%', height: 37*(displayEvent.endIndex -displayEvent.startIndex - (displayEvent.endOffset + displayEvent.startOffset)/hourParts)+'px'}\">                            <div class=\"calendar-event-inner\">{{displayEvent.event.title}}<br>{{displayEvent.event.description}}</div>\n" +
        "                        </div>\n" +
        "                    </div>\n" +
        "                </td>\n" +
